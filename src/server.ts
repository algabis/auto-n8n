...
**Returns**: Tag ID, name, creation/update timestamps`,
  ...
  private async handleWorkflowList(args: unknown) {
    ...
    const workflowSummary = result.data.map((w) => 
      `• **${w.name}** (ID: ${w.id})\n` +
      `  Status: ${w.active ? '🟢 Active' : '🔴 Inactive'}\n` +
      `  Nodes: ${w.nodes?.length || 0}\n` +
      `  Tags: ${w.tags?.map((t) => t.name).join(', ') || 'None'}\n` +
      `  Created: ${w.createdAt ? new Date(w.createdAt).toLocaleString() : 'Unknown'}`
    ).join('\n\n');

    return createSuccessResponse(
      `Found ${result.data.length} workflows`,
      workflowSummary + (result.nextCursor ? `\n\n📄 Next cursor: ${result.nextCursor}` : '')
    );
  }

  private async handleWorkflowGet(args: unknown) {
    const params = validateAndTransform(
      z.object({ id: workflowIdSchema, excludePinnedData: z.boolean().optional() }), 
      args
    );
    
    const workflow = await this.n8nClient.getWorkflow(params.id, params.excludePinnedData);
    
    const nodesList = workflow.nodes?.map((node) => 
      `  • **${node.name}** (${node.type})\n` +
      `    Position: [${node.position?.join(', ') || 'Not set'}]\n` +
      `    Disabled: ${node.disabled ? 'Yes' : 'No'}`
    ).join('\n') || 'No nodes';
  ...
  private async handleWorkflowCreate(args: unknown) {
  ...
  private async handleWorkflowUpdate(args: unknown) {
    const params = validateAndTransform(
      z.object({ 
        id: workflowIdSchema,
  ...
  private async handleWorkflowDelete(args: unknown) {
  ...
  private async handleWorkflowActivate(args: unknown) {
    const params = validateAndTransform(z.object({ id: workflowIdSchema }), args);
  ...
  private async handleWorkflowDeactivate(args: unknown) {
    const params = validateAndTransform(z.object({ id: workflowIdSchema }), args);
  ...
  private async handleWorkflowTransfer(args: unknown) {
    const params = validateAndTransform(
      z.object({ id: workflowIdSchema, destinationProjectId: z.string() }), 
      args
    );
  ...
  private async handleExecutionList(args: unknown) {
    ...
    return createSuccessResponse(
      `Found ${result.data.length} executions`,
      executionSummary + (result.nextCursor ? `\n\n📄 Next cursor: ${result.nextCursor}` : '')
    );
  }

  private async handleExecutionGet(args: unknown) {
    const params = validateAndTransform(
      z.object({ id: executionIdSchema, includeData: z.boolean().optional() }), 
      args
    );
  ...
  private async handleExecutionDelete(args: unknown) {
  ...
  // Tag Tool Handlers
  private async handleTagList(args: unknown) {
    ...
    const tagSummary = result.data.map((tag) => 
      `• **${tag.name}** (ID: ${tag.id})\n` +
      `  Created: ${tag.createdAt ? new Date(tag.createdAt).toLocaleString() : 'Unknown'}`
    ).join('\n');

    return createSuccessResponse(
      `Found ${result.data.length} tags`,
      tagSummary + (result.nextCursor ? `\n\n📄 Next cursor: ${result.nextCursor}` : '')
    );
  }

  private async handleTagCreate(args: unknown) {
  ...
  private async handleTagUpdate(args: unknown) {
    const params = validateAndTransform(
      z.object({ id: tagIdSchema, name: z.string() }), 
      args
    );
  ...
  private async handleTagDelete(args: unknown) {
  ...
  private async handleWorkflowTagsGet(args: unknown) {
    const params = validateAndTransform(z.object({ id: workflowIdSchema }), args);
  ...
  private async handleWorkflowTagsUpdate(args: unknown) {
    const params = validateAndTransform(
      z.object({ id: workflowIdSchema, tagIds: z.array(z.string()) }), 
      args
    );
  ...
  private async handleVariableList(args: unknown) {
    const params = validateAndTransform(z.object({
      limit: z.number().min(1).max(250).default(100).optional(),
      cursor: z.string().optional()
    }), args);
    ...
    const variableSummary = result.data.map((variable) => 
      `• **${variable.key}** (ID: ${variable.id})\n` +
      `  Value: ${variable.value.length > 50 ? variable.value.substring(0, 50) + '...' : variable.value}`
    ).join('\n');

    return createSuccessResponse(
      `Found ${result.data.length} variables`,
      variableSummary + (result.nextCursor ? `\n\n📄 Next cursor: ${result.nextCursor}` : '')
    );
  }

  private async handleVariableCreate(args: unknown) {
  ...
  private async handleVariableUpdate(args: unknown) {
    const params = validateAndTransform(
      z.object({ id: variableIdSchema, key: z.string(), value: z.string() }), 
      args
    );
  ...
  private async handleVariableDelete(args: unknown) {
  ...
  private async handleProjectList(args: unknown) {
    const params = validateAndTransform(z.object({
      limit: z.number().min(1).max(250).default(100).optional(),
      cursor: z.string().optional()
    }), args);
    ...
    const projectSummary = result.data.map((project) => 
    ...
    return createSuccessResponse(
      ...
      projectSummary + (result.nextCursor ? `\n\n📄 Next cursor: ${result.nextCursor}` : '')
    );
  }

  private async handleProjectCreate(args: unknown) {
  ...
  private async handleProjectUpdate(args: unknown) {
    const params = validateAndTransform(
      z.object({ id: projectIdSchema, name: z.string() }), 
      args
    );
  ...
  private async handleProjectDelete(args: unknown) {
  ...
  private async handleCredentialCreate(args: unknown) {
    ...
    return createSuccessResponse(
      `Credential "${credential.name}" created successfully`,
      `ID: ${credential.id}\nType: ${credential.type}`
    );
  }

  private async handleCredentialDelete(args: unknown) {
  ...
  private async handleCredentialSchemaGet(args: unknown) {
    const params = validateAndTransform(
      z.object({ credentialTypeName: z.string() }), 
      args
    );
    ...
    return createSuccessResponse(
      `Schema for credential type "${params.credentialTypeName}"`,
      `\`\`\`json\n${JSON.stringify(schema, null, 2)}\n\`\`\``
    );
  }

  private async handleCredentialTransfer(args: unknown) {
    const params = validateAndTransform(
      z.object({ id: credentialIdSchema, destinationProjectId: z.string() }), 
      args
    );
  ...
  // Audit Tool Handlers
  private async handleAuditGenerate(args: unknown) {
      ...
      if (report.sections) {
        report.sections.forEach((section: any) => {
          ...
          if (section.location && section.location.length > 0) {
            ...
            section.location.forEach((loc: any) => {
              if (loc.workflowName) {
